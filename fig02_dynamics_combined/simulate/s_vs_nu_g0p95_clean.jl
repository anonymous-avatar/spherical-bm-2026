# Stationary s_∞(ν) sweep at γ = 0.95 (same protocol as s_vs_nu_clean.jl).
...
# ν_c via bisection on F0 sign change
νc = NaN
sgn0 = F0_v[1] > 0
sgnE = F0_v[end] > 0
if sgn0 != sgnE
...
println("  ν_c = ", isnan(νc) ? "NaN" : string(round(νc; sigdigits=5)))
...
out = joinpath(datadir, "20260505_s_vs_nu_g0p95_clean.jld2")
jldsave(out;
    ν     = ν_sorted,
    s_inf = s_inf_v,
    μ_inf = μ_inf_v,
